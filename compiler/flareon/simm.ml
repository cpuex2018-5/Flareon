...
and g' env lenv fenv = function (* 各命令の12bit即値最適化 *)
  ...
  | Sw(`V(x), `V(z), y) when M.mem z env  -> if M.mem x env && M.find x env = 0 then Sw(`Zero, `C(M.find z env), y)  else Sw(`V(x), `C(M.find z env), y)
  | Sw(`V(x), `V(z), y) when M.mem z lenv -> if M.mem x env && M.find x env = 0 then Sw(`Zero, `L(M.find z lenv), y) else Sw(`V(x), `L(M.find z lenv), y)
  | Sw(`V(x), `V(z), y) when M.mem y lenv -> if M.mem x env && M.find x env = 0 then Sw(`Zero, `L(M.find y lenv), z) else Sw(`V(x), `L(M.find y lenv), z)
  | Sw(`V(x), y, z) when M.mem x env -> if M.find x env = 0 then Sw(`Zero, y, z) else Sw(`V(x), y, z)
  ...
  | Flw(x, `C(0)) when M.mem x lenv -> Flw("%zero", `L(M.find x lenv))
  | Fsw(`V(x), `V(z), y) when M.mem z env  -> let x' = if List.mem x fenv then `FZero else `V(x) in Fsw(x', `C(M.find z env), y)
  | Fsw(`V(x), `V(z), y) when M.mem z lenv -> let x' = if List.mem x fenv then `FZero else `V(x) in Fsw(x', `L(M.find z lenv), y)
  | Fsw(`V(x), `V(z), y) when M.mem y lenv -> let x' = if List.mem x fenv then `FZero else `V(x) in Fsw(x', `L(M.find y lenv), z)
  | Fsw(`V(x), `C(0), y) when M.mem y lenv -> let x' = if List.mem x fenv then `FZero else `V(x) in Fsw(x', `L(M.find y lenv), "%zero")
  | Fsw(`V(x), z, y) when List.mem x fenv  -> Fsw(`FZero, z, y)
  ...
  | IfEq(x, `V(y), e1, e2) when M.mem y env && M.find y env >= 0 && M.find y env < 32 -> IfEq(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfLE(x, `V(y), e1, e2) when M.mem y env && M.find y env >= 0 && M.find y env < 32 -> IfLE(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfGE(x, `V(y), e1, e2) when M.mem y env && M.find y env >= 0 && M.find y env < 32 -> IfGE(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfEq(x, `V(y), e1, e2) when M.mem x env && M.find x env >= 0 && M.find x env < 32 -> IfEq(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
  | IfLE(x, `V(y), e1, e2) when M.mem x env && M.find x env >= 0 && M.find x env < 32 -> IfGE(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
  | IfGE(x, `V(y), e1, e2) when M.mem x env && M.find x env >= 0 && M.find x env < 32 -> IfLE(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
