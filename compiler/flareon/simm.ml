...
and g' env lenv fenv = function (* 各命令の12bit即値最適化 *)
  ...
  | Sw(`V(x), y, `V(z)) when M.mem z env  -> if M.mem x env && M.find x env = 0 then Sw(`Zero, y, `C(M.find z env))  else Sw(`V(x), y,`C(M.find z env))
  | Sw(`V(x), y, `V(z)) when M.mem z lenv -> if M.mem x env && M.find x env = 0 then Sw(`Zero, y, `L(M.find z lenv)) else Sw(`V(x), y, `L(M.find z lenv))
  | Sw(`V(x), y, `V(z)) when M.mem y lenv -> if M.mem x env && M.find x env = 0 then Sw(`Zero, z, `L(M.find y lenv)) else Sw(`V(x), z, `L(M.find y lenv))
  ...
  | Fsw(`V(x), y, `C(0)) when M.mem y lenv -> let x' = if List.mem x fenv then `FZero else `V(x) in Fsw(x', "%zero", `L(M.find y lenv))
