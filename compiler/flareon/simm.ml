...
and g' env lenv fenv = function (* 各命令の12bit即値最適化 *)
  ...
  | IfEq(x, `V(y), e1, e2) when M.mem y env && (let n = M.find y env in n >= -16 && n < 16) -> IfEq(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfLE(x, `V(y), e1, e2) when M.mem y env && (let n = M.find y env in n >= -16 && n < 16) -> IfLE(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfGE(x, `V(y), e1, e2) when M.mem y env && (let n = M.find y env in n >= -16 && n < 16) -> IfGE(x, `C(M.find y env), g env lenv fenv e1, g env lenv fenv e2)
  | IfEq(x, `V(y), e1, e2) when M.mem x env && (let n = M.find x env in n >= -16 && n < 16) -> IfEq(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
  | IfLE(x, `V(y), e1, e2) when M.mem x env && (let n = M.find x env in n >= -16 && n < 16) -> IfGE(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
  | IfGE(x, `V(y), e1, e2) when M.mem x env && (let n = M.find x env in n >= -16 && n < 16) -> IfLE(y, `C(M.find x env), g env lenv fenv e1, g env lenv fenv e2)
