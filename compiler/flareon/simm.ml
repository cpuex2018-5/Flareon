...
(* TODO *)
let rec g env lenv = function (* 命令列の12bit即値最適化 *)
  | Ans(exp) -> Ans(g' env lenv exp)
  | Let((x, t), Li(i), e) when -2048 <= i && i < 2047 ->
    (* Format.eprintf "found simm12 %s = %d@." x i; *)
    let e' = g (M.add x i env) lenv e in
  ...
  | Let(xt, Sll(y, C(i)), e) when M.mem y env -> (* for array access *)
    ...
    g env lenv (Let(xt, Li((M.find y env) lsl i), e))
  | Let((x, t), SetDL(l), e) ->
    let e' = g env (M.add x l lenv) e in
    if List.mem x (fv e') then Let((x, t), SetDL(l), e') else e'
  | Let(xt, exp, e) -> Let(xt, g' env lenv exp, g env lenv e)
and g' env lenv = function (* 各命令の12bit即値最適化 *)
  ...
  | IfEq(x, V(y), e1, e2) when M.mem y env -> IfEq(x, C(M.find y env), g env lenv e1, g env lenv e2)
  | IfLE(x, V(y), e1, e2) when M.mem y env -> IfLE(x, C(M.find y env), g env lenv e1, g env lenv e2)
  | IfGE(x, V(y), e1, e2) when M.mem y env -> IfGE(x, C(M.find y env), g env lenv e1, g env lenv e2)
  | IfEq(x, V(y), e1, e2) when M.mem x env -> IfEq(y, C(M.find x env), g env lenv e1, g env lenv e2)
  | IfLE(x, V(y), e1, e2) when M.mem x env -> IfGE(y, C(M.find x env), g env lenv e1, g env lenv e2)
  | IfGE(x, V(y), e1, e2) when M.mem x env -> IfLE(y, C(M.find x env), g env lenv e1, g env lenv e2)
  | IfEq(x, y', e1, e2) -> IfEq(x, y', g env lenv e1, g env lenv e2)
  | IfLE(x, y', e1, e2) -> IfLE(x, y', g env lenv e1, g env lenv e2)
  | IfGE(x, y', e1, e2) -> IfGE(x, y', g env lenv e1, g env lenv e2)
  | e -> e
