...
let rec g (env : letenv) (tenv : tupleenv) (ienv : letenv) (exp : KNormal.t) : KNormal.t =
  match exp with
  ...
  | Let(xt, (Int _ as e1), e2) -> Let(xt, e1, g env tenv ienv e2)
  (* | Let(xt, (Float 0.0 as e1), e2) -> Let(xt, e1, g env tenv ienv e2) *)
  (* | Let((x, t), (Int _ as e1), e2) | Let((x, t), (Float _ as e1), e2) ->
     (match List.find_opt (fun (e, _) -> eq_t e e1) ienv with
     | Some (_, y) -> Let((x, t), Var y, g env tenv ienv e2)
     | None -> Let ((x, t), e1, g env tenv ((e1, x) :: ienv) e2))
     | Let(xt, (App(_) as e1), e2) -> Let(xt, e1, g env tenv [] e2) *)
  | Let((x, t), e1, e2) ->
